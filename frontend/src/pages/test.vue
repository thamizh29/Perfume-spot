        ...
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center space-x-4">
            <button @click="toggleCart" class="relative p-2 text-gray-700 hover:text-purple-600 transition-colors">
              ...
              <span v-if="cartItemsCount > 0" class="absolute -top-1 -right-1 bg-purple-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                {{ cartItemsCount }}
              </span>
            </button>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="flex flex-wrap gap-4 items-center justify-between mb-8">
        <div class="flex flex-wrap gap-4">
          <!-- Gender Filter -->
          <select v-model="selectedGender" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
          ...
          <!-- Brand Filter -->
          <select v-model="selectedBrand" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
            <option value="">All Brands</option>
            <option v-for="brand in brands" :key="brand" :value="brand">{{ brand }}</option>
          </select>
          
          <!-- Price Filter -->
          <select v-model="selectedPriceRange" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
        ...
        <div class="flex flex-col h-full">
          ...
          <div class="flex-1 overflow-y-auto p-4">
            ...
            <div v-else class="space-y-4">
              <div v-for="item in cartItems" :key="item.id" class="flex items-center space-x-4 bg-gray-50 p-3 rounded-lg">
      ...
      <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative bg-white rounded-lg max-w-4xl w-full max-h-screen overflow-y-auto">
          <div class="grid md:grid-cols-2 gap-8 p-6">
            <!-- Product Image -->
            <div>
              <img :src="selectedProduct.image" :alt="selectedProduct.name" class="w-full h-96 object-cover rounded-lg">
            </div>
            ...
            <div>
              ...
              <div class="flex items-center mb-4">
                <div class="flex text-yellow-400">
                  <svg v-for="i in 5" :key="i" class="w-5 h-5" :class="i <= selectedProduct.rating ? 'fill-current' : 'text-gray-300'" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                  </svg>
                </div>
              ...
              <div class="flex items-center space-x-4 mb-6">
                ...
                <span v-if="selectedProduct.originalPrice" class="text-gray-400 line-through text-xl">${{ selectedProduct.originalPrice }}</span>
              </div>
              ...
              <div class="flex space-x-4">
                ...
                >
                  <svg class="w-5 h-5" :class="selectedProduct.isWishlisted ? 'fill-current' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                  </svg>
                </button>
              </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            ...
            <p class="text-gray-300">Your destination for premium fragrances from around the world.</p>
          </div>
          <div>
          ...
          <div>
            ...
            <ul class="space-y-2 text-gray-300">
              <li><a href="#" class="hover:text-white transition-colors">Men's Fragrances</a></li>
              <li><a href="#" class="hover:text-white transition-colors">Women's Fragrances</a></li>
...
const filteredProducts = computed(() => {
  return products.value.filter(product => {
    ...
    const matchesPrice = !selectedPriceRange.value || checkPriceRange(product.price, selectedPriceRange.value)
